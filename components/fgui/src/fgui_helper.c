    ...
    if(x == 0 && pixeldata_skip_w%8 > 0) {
        ...
        if(w>=8) {
            ...
            for(i_h=0;i_h<h;i_h++) {
                ...
                for(i_w=0;i_w<w>>3;i_w++) {
                    ...
                    fgui->fb[((y + i_h) * fgui->bytew) + (x>>3) + i_w] = (getpixelbyte(pixeldata, ((i_h + pixeldata_skip_h) * bytew) + (pixeldata_skip_w>>3) + i_w, fgcolor) << (pixeldata_skip_w%8)) |
        ...
        if(w%8) {
            ...
            if((w%8) + (pixeldata_skip_w%8) > 8) {
                ...
                for(i_h=0;i_h<h;i_h++) {
                    ...
                    fgui->fb[((y + i_h) * fgui->bytew) + (x>>3) + i_w] &= ~bitmask_leftovers;
                    fgui->fb[((y + i_h) * fgui->bytew) + (x>>3) + i_w] |= ((getpixelbyte(pixeldata, ((i_h + pixeldata_skip_h) * bytew) + (pixeldata_skip_w>>3) + i_w, fgcolor) << (pixeldata_skip_w%8)) |
                ...
                for(i_h=0;i_h<h;i_h++) {
                    /* Height/row loop. Per row, copy the leftover byte (the last column, at w/8) */
                    fgui->fb[((y + i_h) * fgui->bytew) + (x>>3) + i_w] &= ~bitmask_leftovers;
                    fgui->fb[((y + i_h) * fgui->bytew) + (x>>3) + i_w] |= (getpixelbyte(pixeldata, ((i_h + pixeldata_skip_h) * bytew) + (pixeldata_skip_w>>3) + i_w, fgcolor) << (pixeldata_skip_w%8)) & bitmask_leftovers;
                }                                                                                    
            }
        }
    }
        ...
        if(w>=8) {
            ...
            for(i_h=0;i_h<h;i_h++) {
                ...
                for(i_w=0;i_w<w>>3;i_w++) {
                    /* Width/column loop. Copy all complete bytes (w=0 to w/8) */
                    fgui->fb[((y + i_h) * fgui->bytew) + (x>>3) + i_w] = getpixelbyte(pixeldata, ((i_h + pixeldata_skip_h) * bytew) + (pixeldata_skip_w>>3) + i_w, fgcolor);
                }
            }
        }
        if(w%8) {
        ...
        if(w>=8) {
            ...
            for(i_h=0;i_h<h;i_h++) {
                ...
                for(i_w=0;i_w<w>>3;i_w++) {
                    /* Width/column loop. Copy all complete bytes (w=0 to w/8) */
                    fgui->fb[((y + i_h) * fgui->bytew) + (x>>3) + i_w] &= ~bitmask_right;
                    fgui->fb[((y + i_h) * fgui->bytew) + (x>>3) + i_w] |= getpixelbyte(pixeldata, ((i_h + pixeldata_skip_h) * bytew) + i_w, fgcolor) >> (x%8);

                    fgui->fb[((y + i_h) * fgui->bytew) + (x>>3) + i_w + 1] &= ~bitmask_left;
                    fgui->fb[((y + i_h) * fgui->bytew) + (x>>3) + i_w + 1] |= getpixelbyte(pixeldata, ((i_h + pixeldata_skip_h) * bytew) + i_w, fgcolor) << (8-(x%8));
                }
            }
        }
        if(w%8) {
            ...
            if((w%8)+(x%8) < 8) {
                ...
                for(i_h=0;i_h<h;i_h++) {
                    /* Height/row loop. Per row, copy the leftover byte (the last column, at w/8) */
                    fgui->fb[((y + i_h) * fgui->bytew) + (x>>3) + i_w] &= ~bitmask_leftovers;
                    fgui->fb[((y + i_h) * fgui->bytew) + (x>>3) + i_w] |= (getpixelbyte(pixeldata, ((i_h + pixeldata_skip_h) * bytew) + i_w, fgcolor) >> (x%8)) & bitmask_leftovers;
                }
            }
            else {
                ...
                for(i_h=0;i_h<h;i_h++) {
                    ...
                    fgui->fb[((y + i_h) * fgui->bytew) + (x>>3) + i_w + 1] &= ~bitmask_leftovers;
                    fgui->fb[((y + i_h) * fgui->bytew) + (x>>3) + i_w + 1] |= (getpixelbyte(pixeldata, ((i_h + pixeldata_skip_h) * bytew) + i_w, fgcolor) << (8-(x%8))) & bitmask_leftovers;
                }
            }
        }
    }
